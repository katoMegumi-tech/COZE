import { View, Text, ScrollView, Image as TaroImage, Textarea } from '@tarojs/components'
...
const CustomCreatePage: FC = () => {
  ...
  const handleImageSelect = async (sourceType: 'album' | 'camera') => {
    try {
      ...
      console.log('[CustomPage] Selected image:', tempFilePath)
      ...
      try {
        ...
        console.log('[CustomPage] Image converted to base64, length:', base64.length)
        ...
        console.error('[CustomPage] Convert error:', convertError)
        Taro.hideLoading()
  ...
  const handleGenerate = async () => {
    ...
    Taro.navigateTo({ url: '/pages/result/index?from=custom' })
  }
    ...
    <View className="min-h-screen bg-black">
      <View className="flex flex-row items-center px-4 py-3 border-b border-gray-800">
        <View className="flex flex-row items-center" onClick={handleBack}>
          ...
          <Text className="text-white text-base ml-1">自定义创作</Text>
        </View>
      </View>
      ...
      <ScrollView scrollY className="px-4" style={{ height: 'calc(100vh - 240px)' }}>
        <View className="mb-4">
          <Text className="text-gray-400 text-xs mb-2">参考图片</Text>
          <View 
        ...
        <View className="mb-4">
          ...
          <View className="bg-gray-800 rounded-xl p-4 mb-2">
            <Textarea
              style={{ width: '100%', minHeight: '120px', backgroundColor: 'transparent', color: '#ffffff', fontSize: '14px' }}
              placeholder="请详细描述你想要生成的视频内容，包括场景、动作、风格等..."
              maxlength={2000}
        ...
        <View className="mb-4">
          ...
          <Text className="text-white text-sm font-medium mb-2">视频格式</Text>
          <View className="flex flex-row gap-2 mb-4">
            <View className="rounded-lg px-4 py-2" style={{ background: formData.videoFormat === 'vertical' ? 'linear-gradient(90deg, #a855f7 0%, #ec4899 100%)' : '#1f2937' }} onClick={() => setFormData({ ...formData, videoFormat: 'vertical' })}>
              <Text className="text-white text-sm">竖屏</Text>
            </View>
            <View className="rounded-lg px-4 py-2" style={{ background: formData.videoFormat === 'horizontal' ? 'linear-gradient(90deg, #a855f7 0%, #ec4899 100%)' : '#1f2937' }} onClick={() => setFormData({ ...formData, videoFormat: 'horizontal' })}>
